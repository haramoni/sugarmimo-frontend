...
const statusOptions: Array<{
  ...
  { value: "PENDING", label: "Aguardando análise", icon: Clock3, stat: "pending" },
  { value: "APPROVED", label: "Aprovadas", icon: ShieldCheck, stat: "approved" },
  { value: "REJECTED", label: "Não aprovadas", icon: ShieldAlert, stat: "rejected" },
];
...
export default function PhotoModerationPage() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/photo-moderation?${params.toString()}`);
      ...
      if (!response.ok) {
        throw new Error(result?.message ?? "Não foi possível carregar a moderação de fotos.");
      }
  ...
  async function reviewPhoto(
    ...
    if (action === "reject") {
      const prompt = await Swal.fire({
        ...
        inputPlaceholder: "Explique objetivamente qual regra a imagem não atende...",
        inputAttributes: { maxlength: "1000", "aria-label": "Motivo da rejeição" },
        ...
        preConfirm: (value) => {
          ...
          if (normalized.length < 8) {
            Swal.showValidationMessage("Informe um motivo claro com pelo menos 8 caracteres.");
            return false;
          }
          return normalized;
        },
    ...
    try {
      ...
      setError(reviewError instanceof Error ? reviewError.message : "Não foi possível revisar a foto.");
    } finally {
      setBusyPhotoId("");
    }
  }
  ...
  async function blockProfile(profile: ModerationProfile) {
    const prompt = await Swal.fire({
      ...
      inputAttributes: { maxlength: "1000", "aria-label": "Motivo do bloqueio" },
      showCancelButton: true,
    ...
    setError("");
    try {
      const response = await fetch(`/api/admin/profiles/${encodeURIComponent(profile.id)}/ban`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reason: String(prompt.value) }),
      });
      const result = await response.json().catch(() => null);
      if (!response.ok) throw new Error(result?.message ?? "Não foi possível bloquear o perfil.");
      ...
      setError(blockError instanceof Error ? blockError.message : "Não foi possível bloquear o perfil.");
    } finally {
      setBusyProfileId("");
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--surface)] px-4 py-8 text-[var(--espresso)] sm:px-8 lg:px-10">
      <div className="mx-auto max-w-[1500px] space-y-6">
        <header className="overflow-hidden rounded-3xl bg-[var(--espresso)] px-6 py-7 text-white shadow-xl sm:px-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">Validação de fotos</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-white/66 sm:text-base">
                Toda imagem nova, privada ou pública, permanece invisível aos demais usuários até ser aprovada.
                A galeria completa de cada perfil aparece abaixo para dar contexto à decisão.
              </p>
            </div>
            <Button
              type="button"
        ...
        <section className="grid gap-3 md:grid-cols-3" aria-label="Estados da moderação">
            ...
            return (
              ...
              >
                <span className="flex items-center gap-3">
                  <span className={`grid h-10 w-10 place-items-center rounded-xl ${active ? "bg-white/10" : "bg-[var(--gold)]/12"}`}>
                    <Icon className="h-5 w-5" />
                  </span>
                  <span className="text-sm font-extrabold">{option.label}</span>
                </span>
                <strong className="text-2xl font-black">{stats[option.stat]}</strong>
              </button>
            );
          })}
        </section>

        <form onSubmit={submitSearch} className="flex gap-2 rounded-2xl border border-black/8 bg-white p-3 shadow-sm">
          ...
          <Button type="submit" className="h-11 rounded-xl bg-[var(--espresso)] px-6 text-white hover:bg-[var(--gold)]">
            Buscar
          </Button>
        </form>

        {error ? (
          <div role="alert" className="flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-800">
        ...
        {isLoading ? (
          <div className="grid min-h-72 place-items-center rounded-3xl border border-black/8 bg-white">
            <span className="flex items-center gap-3 text-sm font-bold text-black/50">
              <LoaderCircle className="h-5 w-5 animate-spin" /> Carregando galerias...
            </span>
          </div>
        ) : profiles.length === 0 ? (
          <div className="grid min-h-72 place-items-center rounded-3xl border border-dashed border-black/15 bg-white p-8 text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-black/50">A fila será atualizada quando houver novos envios.</p>
            </div>
          </div>
        ) : (
          <section className="space-y-5" aria-label="Perfis com fotos para moderação">
            {profiles.map((profile) => (
              <article key={profile.id} className="overflow-hidden rounded-3xl border border-black/8 bg-white shadow-sm">
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="truncate text-lg font-black">@{profile.username}</h2>
                      ...
                      <p className="truncate text-xs text-black/48">
                        {profile.email} · {[profile.city, profile.state].filter(Boolean).join(" / ") || "Local não informado"} · {profile.photos.length} foto(s)
                      </p>
                    </div>
                  </div>
                  <Button
                    type="button"
                    variant="outline"
                    disabled={profile.accountStatus === "BANNED" || busyProfileId === profile.id}
                    onClick={() => void blockProfile(profile)}
                    className="border-red-200 text-red-800 hover:bg-red-50 hover:text-red-900"
                  >
                    {busyProfileId === profile.id ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Ban className="h-4 w-4" />}
                    {profile.accountStatus === "BANNED" ? "Acesso bloqueado" : "Bloquear acesso"}
                  </Button>
                </div>
                ...
                <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                  {profile.photos.map((photo, index) => {
                    ...
                    return (
                      <div key={photo.id} className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm">
                        <div className="relative aspect-[4/3] overflow-hidden bg-[#e9e3d9]">
                          ...
                          <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-2 bg-gradient-to-b from-black/65 to-transparent p-3">
                            <span className="inline-flex items-center gap-1 rounded-full bg-black/55 px-2 py-1 text-[10px] font-extrabold uppercase text-white backdrop-blur-sm">
                              {photo.isPrivate ? <Lock className="h-3 w-3" /> : <Images className="h-3 w-3" />}
                        ...
                        <div className="space-y-3 p-3.5">
                          <div>
                            <p className="truncate text-xs font-extrabold">{photo.fileName || `Foto ${index + 1}`}</p>
                            <p className="mt-1 text-[11px] text-black/45">Enviada em {formatDateTime(photo.createdAt)}</p>
                          </div>
                          ...
                          {isPending ? (
                            <div className="grid grid-cols-2 gap-2">
                              <Button
                                type="button"
                                onClick={() => void reviewPhoto(profile, photo, "approve")}
                                disabled={Boolean(busyPhotoId)}
                                className="bg-emerald-700 text-white hover:bg-emerald-800"
                              >
                                {busyPhotoId === photo.id ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Aprovar
                              </Button>
                              <Button
                                type="button"
                                variant="outline"
                                onClick={() => void reviewPhoto(profile, photo, "reject")}
                                disabled={Boolean(busyPhotoId)}
                                className="border-red-200 text-red-800 hover:bg-red-50 hover:text-red-900"
                              >
        ...
        {pagination.totalPages > 1 ? (
          <nav className="flex items-center justify-between rounded-2xl border border-black/8 bg-white p-3" aria-label="Paginação da moderação de fotos">
            <Button type="button" variant="outline" disabled={!pagination.hasPreviousPage || isLoading} onClick={() => setPage((current) => Math.max(1, current - 1))}>
              <ChevronLeft className="h-4 w-4" /> Anterior
            </Button>
            <span className="text-xs font-bold text-black/55">Página {pagination.page} de {pagination.totalPages}</span>
            <Button type="button" variant="outline" disabled={!pagination.hasNextPage || isLoading} onClick={() => setPage((current) => current + 1)}>
...
function StatusBadge({ status }: { status: ModerationStatus }) {
  ...
  const labels = { PENDING: "Em análise", APPROVED: "Aprovada", REJECTED: "Não aprovada" };

  return (
    <span className={`rounded-full px-2 py-1 text-[10px] font-extrabold uppercase ${styles}`}>
      {labels[status]}
    </span>
  );
}
